...
constexpr float distance(ivec2& a, ivec2& b){return sqrt((a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y));}
...
constexpr float interpolateLinear(float valA, float valB, fvec2 posA, fvec2 posB, fvec2 posC){
	float valDiff = valB-valA;
	return valA + (valDiff*(posC.x-posA.x)+valDiff*(posC.y-posA.y))/((posB.x-posA.x)+(posB.y-posA.y));
}

constexpr float interpolateLinear(float start, float end, float t){
    return start+(end-start)*t;
}

constexpr WORD interpolateLinear(WORD start, WORD end, float t){
    return start+(end-start)*t;
}
