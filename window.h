...
ErrCode copyImageToWindow(Window* window, Image& image, WORD start_x, WORD start_y, WORD end_x, WORD end_y, float opacity = 1.0)noexcept{
	#ifdef INVALIDHANDLEERRORS
	if(window == nullptr) return WINDOW_NOT_FOUND;
	#endif
	WORD buffer_width = window->windowWidth/window->pixelSize;
	for(int y=start_y; y < end_y; ++y){
		...
		for(int x=start_x; x < end_x; ++x){
			...
			if(A(color) > 0){
				DWORD currentColor = window->pixels[y*buffer_width+x];
				window->pixels[y*buffer_width+x] = RGBA(R(color)*opacity+R(currentColor)*(1.f-opacity), G(color)*opacity+G(currentColor)*(1.f-opacity), B(color)*opacity+B(currentColor)*(1.f-opacity));
			}
		}
	}
	return SUCCESS;
...
//TODO man sollte texturen auf objekt level speichern, also das erste TODO
